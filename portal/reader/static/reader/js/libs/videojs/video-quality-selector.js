...
(function( _V_ ) {
	...
	_V_.ResolutionSelector = _V_.MenuButton.extend({
		...
		init : function( player, options ) {
            ...
            this.el().firstChild.firstChild.className = " vjs-quality-text";
            this.el().firstChild.firstChild.style.cssText = "font-family: open_sansbold, sans-serif; font-size: 14px !important; line-height: 30px !important;";
		}
	});
	...
	_V_.ResolutionSelector.prototype.createItems = function() {
        ...
        //console.log(player.availableRes);
        var tempAvailableRes = [];
        for ( avRes in player.availableRes ) {
            if ( 'length' == avRes ) { continue; }
            tempAvailableRes.push(player.availableRes[avRes]);
        };
        tempAvailableRes.sort(function(a, b){
            return parseInt( b[0]['data-level'] ) - parseInt( a[0]['data-level'] );
        });
        var tempResObject = {};
        for ( res in tempAvailableRes ) {
            tempResObject[tempAvailableRes[res][0]['data-res']] = tempAvailableRes[res];
        }
//        for ( current_res in player.availableRes ) {
		for ( current_res in tempResObject ) {
	...
	_V_.plugin( 'resolutionSelector', function( options ) {
		...
		player.changeRes = function( target_resolution ) {
			...
			if ( player.controlBar.resolutionSelector ) {
				...
				// EPP-7427
				$(player.controlBar.resolutionSelector.el()).find('.vjs-quality-text').text(qualitiies[target_resolution]);
			}
			// Update the classes to reflect the currently selected resolution
			player.trigger( 'changeRes' );
		};

        player.changeStartRes = function( target_resolution ) {
		...
		// EPP-7427
		$(player.controlBar.resolutionSelector.el()).find('.vjs-quality-text').text(qualitiies[current_res]);
	});
