...
function showHint(item_id) 
{
    ...
    if (hints[thehint]) {
        ...
        // EPP-6767
        $('#' + item_id).find('div.hint').hide();
        resetHintCounter(item_id);
    }
}
...
function resetHintCounter(item_id)  // EPP-6767
{
    counternum = 0;
    while (counternum < hintcounter.length)
        {
            ...
            if (hintid == item_id)
                {
                    hintcounter[counternum].hintnum = 0;
                }
            counternum++;
        }
}
...
function getSolutionButton(id,whichSolution,whichAction) {
    /*
    descendantSpans = document.getElementById(id).getElementsByTagName("input");
    ...
    var inputs = jquery(jquery('#'+id).find('.solution-toggles')[whichSolution - 1]).find('input');
    if (whichAction == 'show') {
        return inputs[0]; // there are 2 "buttons" for each solution. take the first of each pair.
    } else {
        return inputs[1]; // there are 2 "buttons" for each solution.  take the second of each pair.
    }
}
...
function hideSolutions(){
  ...
  var y = document.getElementsByClassName('solution-toggles');
  for (var i = 0; i < y.length; i++) {
    y[i].style.display = 'block';
  }
}

function getExerciseCommentary(id,whichSolution) {
    descendantDivs = document.getElementById(id).getElementsByTagName("div");
    solutionDivs = new Array();
    for (i = 0, j = 0; i < descendantDivs.length; i++) {
	if (descendantDivs[i].className == 'commentary' || descendantDivs[i].className == 'commentary commentary-example') {
	    solutionDivs[j]=descendantDivs[i];
	    j++;
	}
    }
...
function getCommentaryButton(id,whichSolution,whichAction) {
    /*descendantSpans = document.getElementById(id).getElementsByTagName("input");
    buttonSpans = new Array();
    for (i = 0, j = 0; i < descendantSpans.length; i++) {
	if (descendantSpans[i].className == 'commentary-toggle') {
	    buttonSpans[j]=descendantSpans[i];
	    j++;
	}
    }
    ...
    var jquery = require('jquery');
    var inputs = jquery(jquery('#'+id).find('.commentary-toggles')[whichSolution - 1]).find('input');
    if (whichAction == 'show') {
        return inputs[0]; // there are 2 "buttons" for each solution. take the first of each pair.
    } else {
        return inputs[1]; // there are 2 "buttons" for each solution.  take the second of each pair.
...
function toggleCommentary(id,whichSolution) {
    // If showing the solution
    if (getExerciseCommentary(id,whichSolution).style.display != 'block') {
        getExerciseCommentary(id,whichSolution).style.display="block";
        getCommentaryButton(id,whichSolution,"show").style.display="none";
        getCommentaryButton(id,whichSolution,"hide").style.display="inline";
    // If hiding the solution
    } else {
        getExerciseCommentary(id,whichSolution).style.display="none";
        getCommentaryButton(id,whichSolution,"show").style.display="inline";
        getCommentaryButton(id,whichSolution,"hide").style.display="none";
    }
}

function hideCommentaries(){
    var x = document.getElementsByClassName('commentary');
    for (var i = 0; i < x.length; i++) {
        x[i].style.display = 'none';
    }
    var y = document.getElementsByClassName('commentary-toggles');
    for (i = 0; i < y.length; i++) {
        y[i].style.display = 'block';
    }
}

function getExerciseNote(id, whichSolution) {
    return document.getElementById(id);
}


function getNoteButton(id, whichSolution, whichAction) {
    var inputs = document.getElementById(id + "_toggle").getElementsByTagName("input");
    if (whichAction == 'show') {
        return inputs[0]; // there are 2 "buttons" for each note. take the first of each pair.
    } else {
        return inputs[1]; // there are 2 "buttons" for each note.  take the second of each pair.
    }
}

function toggleNote(id, whichSolution) {
    // If showing the note
    if (getExerciseNote(id, whichSolution).style.display != 'block') {
        getExerciseNote(id, whichSolution).style.display = "block";
        getNoteButton(id, whichSolution, "show").style.display = "none";
        getNoteButton(id, whichSolution, "hide").style.display = "inline";
        // If hiding the note
    } else {
        getExerciseNote(id, whichSolution).style.display = "none";
        getNoteButton(id, whichSolution, "show").style.display = "inline";
        getNoteButton(id, whichSolution, "hide").style.display = "none";
    }
}

function hideNotes() {
    var noteTips = document.getElementsByClassName('note-tip');

    for (var i = 0; i < noteTips.length; i++) {
        if (noteTips[i].parentNode.className.split(' ').indexOf('exercise-contents') >= 0) {
            noteTips[i].style.display = 'none'; //only in exercise
        }
    }
    var noteToggles = document.getElementsByClassName('note-toggle');
    for (i = 0; i < noteToggles.length; i++) {
        if (noteToggles[i].parentNode.className.split(' ').indexOf('exercise-contents') >= 0) {
            noteToggles[i].style.display = 'block'; //only in exercise
        }
    }
}
