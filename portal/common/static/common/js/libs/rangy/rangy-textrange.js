...
(function(factory, root) {
    ...
    rangy.createModule("TextRange", ["WrappedSelection"], function(api, module) {
        ...
        function isHidden(node) {
            var ign = isIgnored(node);
            if(inFoldable(node) &&!ign){
                return false;
            }
        ...
        var checkElement = function(el){
            if(api.config.ignoredClasses && api.config.ignoredClasses.constructor===Array){
                for(var i=0, ignoredClass;ignoredClass=api.config.ignoredClasses[i];i++){
        ...
        //dodatkowy warunek dla foldable-contents
        var inFoldable = function(node){
            while(node){
                if(node.nodeType===1){
                    if(hasClass(node,"foldable-contents")){
                        return true;
                    }
                }
        ...
        function createCharacterIterator(startPos, backward, endPos, characterOptions) {
            ...
            function checkMathJax(posit, which){
                if(posit && posit.node && posit.node[which]){
                    if(api.config['includeAttrCutoff_' + which]) {
                        for (var i = 0, ignoredClass; ignoredClass = api.config['includeAttrCutoff_' + which][i]; i++) {
                            if(posit.node[which].indexOf(ignoredClass) > -1){
                                return true;
                            }
                        }
                    }
            ...
            function next() {
                var charPos = null;
                if(checkMathJax(pos, 'className') || checkMathJax(pos, 'id') || checkMathJax(endPos, 'className') || checkMathJax(endPos, 'id')){
