define([
    ...
    function generateHeader(coll, module){
        return coll.md_title + ' - ' + module.md_title;
    }
    ...
    var NoteModel = Backbone.Model.extend({
        ...
        getViewAttrs: function(){
            var relatedCollection = this.get(RELATED_PREFIX + 'Collection');
            var pattern = $('.notes-list').data('url-pattern');
            ...
            return {
                header: generateHeader(relatedCollection, relatedCollection[RELATED_PREFIX + 'Module']),
                text: this.get('text'),
                subject: this.get('subject'),
                date: makeDate(new Date(this.get('modify_time'))),
                content: content,
                color: 1 + this.get('type'),
                read_link: link
            }
        }
    });
