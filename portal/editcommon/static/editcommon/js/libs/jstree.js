 ...
 */
	...
	$.jstree.core.prototype = {
		...
		redraw_node : function (node, deep, is_callback, force_render) {
			...
			node = !document.querySelector ? document.getElementById(obj.id) : this.element ? this.element[0].querySelector('#' + ("0123456789".indexOf(obj.id[0]) !== -1 ? '\\3' + obj.id[0] + ' ' + obj.id.substr(1).replace($.jstree.idregex,'\\$&') : obj.id.replace($.jstree.idregex,'\\$&')) ) : null; //, this.element);
            if(!node) {
				deep = true;
